...
const ONSCREEN_AGENT_AVATAR_PATH = "/mod/_core/onscreen_agent/res/helmet_no_bg_256.webp";
...
function createMessageAvatar(role) {
  ...
  if (role === "assistant") {
    ...
    image.src = ONSCREEN_AGENT_AVATAR_PATH;
    image.alt = "";
    image.loading = "lazy";
    avatar.append(image);
    return avatar;
...
export function renderMessages(thread, history, options = {}) {
  ...
  if (!renderedMessages.length) {
    ...
    hint.textContent = "Send a message to start the onscreen agent thread.";
  ...
  buildMessageDisplayGroups(renderedMessages, options).forEach((group) => {
    if (group.type === "assistant-sequence") {
      ...
      if (group.message.role === "assistant" && group.message.streaming) {
...
export function updateStreamingAssistantMessage(thread, message, options = {}) {
