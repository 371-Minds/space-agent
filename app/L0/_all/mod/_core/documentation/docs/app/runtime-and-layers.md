...
- `space.browser`: web-browsing helper surface for open browser windows; the public agent-facing API uses numeric ids such as `1` even though the internal store still keeps `browser-N` ids, and it exposes `ids()`, `list()`, `count()`, `has(id)`, `state(id)`, and `current()` for discovery, top-level `open(...)` and `create(...)` that return a per-window handle for the new browser modal, `close(id)` and `closeAll()` for teardown, `send(browserId, type, payload)`, convenience wrappers `dom(id, ...)`, `content(id, ...)`, and `detail(id, ref, ...)`, `sync(browserId, ...)`, direct navigation helpers keyed by browser id, and ref-targeted helpers such as `click(id, ref)`, `type(id, ref, value)`, `submit(id, ref)`, `typeSubmit(id, ref, value)`, and `scroll(id, ref)`, plus `get(browserId)` handles with matching per-window helpers; injected browser pages currently support `ping`, raw `dom`, stateful reference-marked `content`, `detail`, `click`, `type`, `submit`, `type_submit`, `scroll`, and navigation bridge requests, and those agent-facing methods are currently guarded through one module-owned availability stub that resolves native-app runtime from the framework context plus the explicit desktop bridge, returning a structured warning in plain browser runtime instead of attempting native-app-only browser actions
