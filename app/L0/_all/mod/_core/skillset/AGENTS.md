...
- `ext/skills/mjunkie-simulator/SKILL.md`: the top-level on-demand skill for Multimedia Junkie simulator invocation and deterministic Space Agent versus simulator memory benchmarking without touching `~/memory`
...
- this module owns repo-owned shared first-party top-level skills such as `development`, `browser-manager`, `browser-control`, `file-download`, `mjunkie-simulator`, `pdf-report`, `screenshots`, and `user-management`; module-specific skills that describe one module's private contracts may still live under that owning module
...
- the `mjunkie-simulator` skill must keep the separation contract explicit: Space Agent memory remains under `~/memory/...`, simulator memory stays external, and benchmark flows compare separate `entries` arrays without teaching any silent merge or replacement path
