...
- `app/L0/_all/mod/_core/visual/`: shared visual asset aggregator for reusable canvas, chrome, button, dialog, surface, and conversation-thread styles plus small dialog helpers; shared look-and-feel work belongs here before it belongs in feature-local CSS
...
- `app/L0/_all/mod/_core/onscreen_menu/`: floating top-right menu button injected into the router shell via the `_core/router/shell_start` anchor; provides Admin and Logout navigation for all router-hosted views
...
- `app/L0/_all/mod/_core/admin/`: current firmware-backed admin UI, organized by surface under `views/shell/`, `views/dashboard/`, `views/agent/`, `views/modules/`, and `views/documentation/`, with admin-only prompt skills under `skills/` and the split shell mounted through a page-specific extension anchor
